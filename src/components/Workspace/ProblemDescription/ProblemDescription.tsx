      ...
      if (userDoc.exists() && problemDoc.exists()) {
        if (liked) {
          ...
          transaction.update(userRef, { 
            ...
            dislikedProblems: userDoc.data().dislikedProblems.filter((item: string) => item !== problem.id)
          })
  ...
  const handleDislike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        ...
        if (disliked) {
          transaction.update(userRef, { dislikedProblems: userDoc.data().dislikedProblems.filter((id: string) => id !== problem.id) })
          transaction.update(problemRef, { dislikes: problemDoc.data().dislikes - 1 })
						...
						<div className='mt-4'>
              {
                problem.examples.map((example, index) => (
                  <div key={example.id}>
                    ...
                    {
                      example.img && (
                        <img className="mt-3" src={example.img} />
                      )
                    }
